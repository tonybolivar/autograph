...
const URL_ = 'https://ats.rippling.com/etg/jobs/d34b3e22-3172-4db1-9d70-b1441b785f5d/apply?step=application';

const PROFILE = {
  first_name: 'Anthony', last_name: 'Bolivar', email: 'tony.e.bolivar@gmail.com',
  phone_number: '(936) 419-2746', phone_type: 'Mobile', address_line_1: '123 Main St',
...
async function main() {
  ...
  const r = await page.evaluate(() => {
    ...
    const entries = inputs.map(el => {
      ...
      const lab = (labelEl?.textContent || '').trim().slice(0, 50);
      return { t, name: name.slice(0, 40), placeholder: ph.slice(0, 40), label: lab, v: typeof v === 'string' ? v.slice(0, 50) : v, ag };
    });
    return {
  ...
  console.log('BODY_LEN=', r.bodyLen);
  console.log('=== RIPPLING FILLED ===', r.filledCount, '/', r.total);
  for (const x of r.entries) {
    const flag = x.ag ? '[F]' : (x.v ? '[v]' : '   ');
    console.log(`${flag} ${(x.t || '').padEnd(10)} ${(x.label || x.placeholder || x.name).padEnd(40)} | ${JSON.stringify(x.v)}`);
  }

...
const URL_ = 'https://greatminds.recruitee.com/o/staff-software-engineer/c/new';
...
async function main() {
  ...
  const r = await page.evaluate(() => {
    ...
    for (const el of fields) {
      ...
      const e = { t, ref: (name || id).slice(0, 50), label: lbl, v: typeof v === 'string' ? v.slice(0, 50) : v, ag };
      if (isCheck && el.checked) out.checked.push(e);
      else if (ag || (typeof v === 'string' && v)) out.filled.push(e);
      else if (lbl) out.unfilled.push(e);
    }
    const resume = document.querySelector('input[type="file"][name="candidate.cv"]');
    out.filledCount = document.querySelectorAll('[data-ag-filled="true"]').length;
    out.resumeFiles = resume ? resume.files.length : null;
    return out;
  });

  console.log('=== RECRUITEE FILLED ===', r.filledCount, 'resume:', r.resumeFiles);
  console.log('\n=== FILLED ===');
  for (const f of r.filled) console.log(`[F] ${f.t.padEnd(10)} ${(f.label || f.ref).padEnd(50)} | ${JSON.stringify(f.v)}`);
  console.log('\n=== CHECKED ===');
  for (const f of r.checked) console.log(`[C] ${f.t.padEnd(10)} ${(f.label || f.ref)}`);
  console.log('\n=== UNFILLED (first 15) ===');
  for (const f of r.unfilled.slice(0, 15)) console.log(`    ${f.t.padEnd(10)} ${f.label.padEnd(50)} | ${f.ref.slice(0, 30)}`);

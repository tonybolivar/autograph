...
- **Paylocity** (recruiting.paylocity.com): tenant URLs are short-lived hashes (`/All/<uuid>/Tenant`) and are rarely indexed by search engines. The few discoverable URLs return JobNotFound. Live probing requires receiving an active tenant URL from a user; the adapter is structurally complete and exercises pl-select dropdowns via mousedown.
- **Bullhorn** (bullhornstaffing.com): search engines do not surface live job URLs on this host. The major routes (auth, app, login) are recruiter-facing portals, not candidate apply forms. Public-facing roles are usually hosted on a tenant subdomain that we cannot discover without referral; the adapter is structurally complete.
- **UKG** (recruiting.ultipro.com): tenant slugs are opaque (e.g. `/MED1095/JobBoard/<guid>`) and not indexed; we cannot discover a live posting from search engines. Adapter is structurally complete.
- **Workday** confirmed gating (2026-05-13): NVIDIA tenant at `nvidia.wd5.myworkdayjobs.com` reaches the per-job `/apply` page which shows "Start Your Application" with Autofill / Apply Manually / Use My Last Application buttons. The form fields are not in the DOM until the user clicks through account creation or sign-in.
- **SuccessFactors** confirmed gating (2026-05-13): `career5.successfactors.eu/career?company=Roche` renders an empty SPA body when accessed unauthenticated; the same pattern holds for other tenants we attempted. Form fields are gated.
- **Oracle HCM** confirmed gating (2026-05-13): `eeky.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/...` renders an Error page when the candidate experience is hit without a tenant-specific path that includes the auth handshake.
- **iCIMS** confirmed gating (2026-05-13): every iCIMS subdomain we tested returns `gone: <subdomain>.icims.com : dc409` from our test environment. Whether that is a network-level block or tenant decommissioning, the apply form is not reachable in the harness today.
- **Eightfold privacy modal loop** (2026-05-13): some tenants (e.g. Accenture) show a Data Privacy modal that re-appears if the form is interacted with before consent is fully recorded. Mitigations in adapter: `waitForReady` delays the first fill, modal/dialog containers are excluded from fill, refills are suppressed on rerender, and consent/privacy/i-agree checkboxes are explicitly excluded.
- **Eightfold multi-step contact info** (2026-05-13): the Email and City fields render at off-screen coordinates (y=-2017 in our test) on the Accenture tenant when the page mounts in the harness. First name/Last name/Phone fill correctly. The off-screen positioning is likely a multi-step form where the next step is pre-rendered but offset; live extension users see them normally.
